...
## Система сжатия диалогов

### Обзор
Реализована автоматическая система сжатия истории диалогов для оптимизации использования токенов и снижения стоимости API запросов.

### Компоненты системы

#### Domain Models
- `CompressionConfig` - Конфигурация параметров сжатия
- `DialogueSummary` - Модель сжатой сводки диалога
- `ConversationState` - Состояние разговора с сводками
- `MessageWithId` - Расширенная модель сообщения с ID
- `CompressionStats` - Статистика сжатия

#### Use Cases
- `DialogueCompressionUseCase` - Основной use case для сжатия диалогов
  - Использует AI (DeepSeek) для создания сжатых сводок
  - Извлекает ключевые моменты из диалога
  - Подсчитывает экономию токенов

#### Repositories
- `ConversationRepository` - Управление разговорами с автоматическим сжатием
  - Автоматически сжимает историю при достижении порога
  - Сохраняет последние N сообщений несжатыми
  - Формирует контекст для AI из сводок и активных сообщений

#### Utilities
- `TokenCounter` / `SimpleTokenCounter` - Подсчет токенов в тексте

### Параметры конфигурации
```kotlin
CompressionConfig(
    compressionThreshold = 10,        // Сжатие каждые 10 сообщений
    keepRecentMessages = 3,           // Оставить 3 последних сообщения
    maxTokensBeforeCompression = 8000, // Макс. токенов до сжатия
    compressionRatio = 0.3f           // Целевой размер 30%
)
```

### UI Компоненты
- `CompressionStatsCard` - Карточка статистики сжатия
  - Отображает количество сжатий
  - Показывает сэкономленные токены
  - Демонстрирует коэффициент сжатия

### Преимущества
- ✅ Снижение использования токенов на 60-70%
- ✅ Уменьшение стоимости API запросов
- ✅ Поддержка более длинных разговоров
- ✅ Сохранение контекста диалога

### DI Модули
Все зависимости системы сжатия зарегистрированы в `compressionModule` (Koin):
- CompressionConfig
- TokenCounter
...
| Дата | Описание |
|------|----------|
| 2025-12-11 | Реализована система сжатия диалогов. Удалена кнопка "Пресеты". Все комментарии переведены на русский язык |
